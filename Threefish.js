...
// se asigna en la variable tf la configuración que tendrá threefish, es decir, la llave que se usará
// la llave debe ser de 32 bytes, a diferencia del código en python
var tf = new Threefish256("key of 32,64 or 128 bytes lenght");
// la variable iv no es lo mismo que el tweak en python, pero se define lo mismo para comprobar
var iv = "tweak: 16 bytes ";

// se encripta el mismo mensaje que se encripto en python para comprobar si se genera el mismo encriptado
var cryptotext = tf.encryptAuthenticated("block of data,same length as key", iv);

// se envía una alerta con el texto encriptado para comprobar el funcionamiento
alert(cryptotext)

// se desencripta para comprobar el funcionamiento del algoritmo y se devuelve el valor por consola
var plaintext1 = tf.decryptAuthenticated(cryptotext, iv);
console.log(plaintext2)

// se obtiene el valor del id y se devuelve por consola el id recibido
var id=document.getElementsByClassName("algorithm").valueOf("id")[0].id

console.log(id)

// se intenta desencriptar lo que contiene el id, primero llamando directamente a decryptCBC
var plaintext2 = tf.decryptCBC(id, iv);
console.log(plaintext1)

// se intenta desencriptar usando la funcion decryptAuthenticated
var plaintext3 = tf.decryptAuthenticated(id, iv);
console.log(plaintext1)

// aquí la comprobación si esque el algoritmo de python y el algoritmo de javascript generan
// el mismo encriptado para los mismos valores
if(cryptotext==id){
    alert("coinciden2")
}
else{
